...
onMounted(() => {
    if (process.client) {
        usetheme.currentTheme = localStorage.getItem('theme') || 'light'
        usetheme.applyTheme(usetheme.currentTheme)
    }
})
...
if (process.client) {
    ...
    mediaQuery.addEventListener('change', (e) => {
        if (usetheme.currentTheme === 'system') {
            usetheme.applyTheme('system')
        }
    })
}
</script>
...
<template>
    <div class="dropdown dropdown-end" :class="dropdownPosition">
        <label tabindex="0" class="btn btn-ghost btn-circle">
            <svg class="w-5 h-5 fill-current" viewBox="0 0 24 24" v-if="usetheme.currentTheme === 'light'">
                <path
        ...
        <ul tabindex="0" class="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-52">
            <li>
                <button @click="usetheme.setTheme('light')" :class="{ 'active': currentTheme === 'light' }">
            ...
            <li>
                <button @click="usetheme.setTheme('dark')" :class="{ 'active': currentTheme === 'dark' }">
                    <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 24 24">
                        <path
                            d="M21.752 15.002A9.718 9.718 0 0118 15.75c-5.385 0-9.75-4.365-9.75-9.75 0-1.33.266-2.597.748-3.752A9.753 9.753 0 003 11.25C3 16.635 7.365 21 12.75 21a9.753 9.753 0 009.002-5.998z" />
                    </svg>
